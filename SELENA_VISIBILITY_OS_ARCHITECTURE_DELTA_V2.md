...
Статус: `APPROVED — 2026-08-28`, утверждено владельцем (Selena).

Кода по этой Delta не написано. Утверждение открывает работу по пакетам M0–M6 и
снимает предусловие `SELENA_VISIBILITY_OS_TZ_CODEX_V2.md` §Предусловие. Оно
**не** является разрешением применять миграции, включать живые адаптеры,
расширять `ownerApprovedMeasurementAdapters`, тратить деньги провайдера или
трогать продовые флаги: запреты §15 и §17 остаются в силе полностью.
...
## 16. Решения владельца

### 16.1 Принято 2026-08-28

**Пункт 2. Провайдер локального ранга.** DataForSEO Standard, тариф $0,0006 за
вызов. Тариф является входом расчёта и параметром функции; в коде он не
появляется ни при каких условиях — запрет `SELENA_VISIBILITY_OS_TZ_M0_ECONOMICS_V2.md`
§3.2 остаётся в силе, и тест-греп на числовой литерал-тариф продолжает быть
частью приёмки M0.

Внесение провайдера в `ownerApprovedMeasurementAdapters` этим решением **не**
производится. Это отдельный акт владельца вместе с креденшелами и провайдерным
капом; до него адаптер локального ранга остаётся инертным, а registry воркера —
`noop`.

**Пункт 3. Потолок точек grid и потолок стоимости цикла.** Потолок — 49 точек
(7×7), он же `DEFAULT_GRID_POINT_CEILING`. Кап стоимости локального цикла — $3,
сравнивается с worst-case, а не с плановой стоимостью
(`assertCycleWithinBudget`, M0 §4).

Проверка согласованности этих чисел на предельной разумной конфигурации:

```text
49 точек × 20 запросов × 2 повтора × 1 провайдер = 1 960 плановых вызовов
плановая стоимость                                = $1,18
worst-case (1 оплачиваемый повтор)  = 3 920 вызовов = $2,35
```

Кап $3 покрывает предельный случай с запасом. Превышение потолка точек остаётся
отдельным решением владельца, а не параметром формы (§12).

### 16.2 Остаётся открытым
...
Ни один из открытых пунктов не блокирует M0 и M2: M0 не касается каталога и
миграций, M2 складывает SQL в `_pending-os/` без применения и не продаётся до
прохождения своих gates. Пункт 1 разрешается владельцем до начала M3, пункт 6 —
до начала работы с доменом репутации в M3.
