...
Статус: `DELTA_APPROVED — 2026-08-28, владелец Selena`. Кода по этой Delta
ещё не написано: утверждение снимает запрет, а не заменяет работу.
...
- потолок точек grid **49 (7×7)** — решено владельцем 28 августа; превышение
  остаётся отдельным решением, а не параметром формы;
- потолок стоимости одного цикла **$3** worst-case — решено там же; кап
...
Решено 28 августа 2026, владелец — Selena. Источник: `HANDOFF_2026-08-27.md`,
раздел «Решения владельца, уже принятые».

| # | Вопрос | Решение |
|---|---|---|
| 3 | Потолок точек grid | **49 (7×7).** Превышение — отдельное решение, а не параметр формы |
| 3 | Потолок стоимости локального цикла | **$3 на цикл** worst-case |
| 3 | Шаг сетки | параметр локации: **400 м** для плотного центра, **800 м** для Чангу |
| 3 | Границы сетки | **одна сетка = один район**, не остров |
| 2 | Провайдер локального ранга | **DataForSEO Standard** |

Тариф провайдера — **$0,0006 за вызов, публичный прайс, счётом не подтверждён.**
Он входит в расчёты как заявленная витрина, а не как проверенный факт, и до
сверки со счётом ни одна оценка на нём не публикуется как стоимость.

При 49 точках × 10 запросов × 2 повтора worst-case — 1960 вызовов. По
непроверенному прайсу это $1,18, то есть кап $3 не достигается. Если счёт
покажет другой тариф, первым пересчитывается кап, а не потолок точек.

### Остаётся открытым

Эти четыре вопроса не решены и блокируют то, что названо рядом, — не M2.

1. Расхождение «ТЗ v1.1» и фактических v1.3/v1.4 (§0). Блокирует только
   ссылочную часть документов.
2. Включение DataForSEO в allowlist адаптеров — операционный шаг, делается
   перед первым живым вызовом, не перед схемой.
3. Изменение каталога под новые пакеты (§13). Блокирует продажу, не разработку.
4. Порядок применения миграций M1–M6 и момент backfill зонтичного реестра.
   Блокирует применение, не написание файлов.
5. Юридический режим сбора отзывов у конкретных площадок. Блокирует
   Reputation-домен.
