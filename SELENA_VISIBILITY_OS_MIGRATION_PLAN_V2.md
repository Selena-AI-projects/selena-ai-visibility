...
Ни один файл миграции по этому плану не написан и не применён. Утверждение
плана не является разрешением на применение: `drizzle-kit migrate` по-прежнему
не запускается ни против одной базы, кроме одноразовой scratch-базы в тестовом
контейнере (§1.1), а применение остаётся актом владельца.
...
2. **Миграции пакетов лежат в `_pending-os/` и не применяются.** Правило §11.2.1
   о присвоении номеров и записи в `_journal.json` одним коммитом на мердже в
   `release` действует без изменений — оно не было свойством ветки.
