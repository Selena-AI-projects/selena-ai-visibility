...
M1 реализован и получил номер миграции, но не применён вне одноразовой
scratch-базы. Утверждение плана и наличие numbered SQL не являются разрешением
на применение: `drizzle-kit migrate` по-прежнему не запускается ни против
одной базы, кроме одноразовой scratch-базы в тестовом контейнере (§1.1), а
применение остаётся актом владельца.

## 0. Проверенный migration head

| Что | Значение |
|---|---|
| Последний файл | `packages/lib/src/db/migrations/0037_visibility_os_measurement_registry.sql` |
| Последняя запись журнала | `_journal.json`, `idx: 37` |
| Первый свободный номер | `0038` |
| Применено к продовой базе | ничего из 0025–0037 |
...
Gate пакета: **Gate 1** (`AI_CORE_NO_REGRESSION_PASS`) и **Gate 2 — M1
registry stage** (`MEASUREMENT_DOMAIN_ISOLATION_PASS`). На этой стадии Gate 2
проверяет реестр, зонтичные циклы, FK evidence и tenant isolation; критерии
Local cost/cardinality/retry/queue впервые проверяются в обязательном повторе
M2.
...
Gate пакета: **Gate 2** (повтор с Local cost/cardinality/retry/queue),
**Gate 3**, **Gate 4**, **Gate 9**.
...
Gate пакета: **Gate 2** (повторно на AI, Local, Search и Reputation),
**Gate 5**.
...
```text
M1 → Gate 1, Gate 2 (M1 registry stage) → остановка и отчёт
M2 → Gate 2 (Local repeat), Gate 3, Gate 4, Gate 9 → остановка и отчёт
M3 → Gate 2 (Search/Reputation repeat), Gate 5
M4 → Gate 6, Gate 7
M5 → Gate 8, Gate 10
...
Visibility OS разрабатывается параллельно и вливается в основную ветку по мере
готовности пакетов.

**Принятый режим — 2026-08-28: §11.1, мердж пакетами по gate.** Долгоживущая
интеграционная ветка `codex/visibility-os` не используется; PR пакетов
нацеливаются напрямую на `release/selena-visibility-mvp`, мердж выполняет
владелец. Правила §11.2 остаются в документе как описание отклонённого режима:
если проект вернётся к накоплению пакетов в одной ветке, они снова становятся
обязательными целиком.

Следствие этого решения для сквозного gate — §11.5. Без него Gate 12 остаётся
без ветки, на которой он был назначен прогоняться.
...
OS-ветка не касается ничего из списка запуска: применения миграций 0025–0037,
...
### 11.5 Где прогоняется Gate 12 в режиме §11.1

`SELENA_VISIBILITY_OS_TZ_CODEX_V2.md` §12.2 назначал Gate 12
(`VISIBILITY_OS_VERTICAL_SLICE_PASS`) интеграционной ветке `codex/visibility-os`.
В принятом режиме этой ветки нет, поэтому место прогона переопределяется:

**Gate 12 прогоняется на `release/selena-visibility-mvp` после мерджа последнего
из пакетов M1–M6, отдельным прогоном, до включения любого флага домена.**

Это не ослабление приёмки. Смысл Gate 12 в том, что сквозной путь проверяется на
собранной системе, а не на отдельном пакете; в режиме §11.1 собранная система —
это и есть `release/selena-visibility-mvp` с влитыми пакетами и выключенными
флагами.

Четыре условия, которые режим §11.1 обязан удерживать вместо ветки:

1. **Мердж пакета не включает ни одного флага.** Новые переменные остаются с
   дефолтом «выключено». Иначе продукт увидит недоделанный домен раньше Gate 12,
   и изоляцию, которую в §11.2 давала ветка, не даст ничто.
2. **До мерджа миграции пакетов лежат в `_pending-os/`; на мердже получают
   номер и запись в `_journal.json`, после мерджа остаются неприменёнными.**
   Правило §11.2.1 действует без изменений — оно не было свойством ветки.
3. **Gate 1 (`AI_CORE_NO_REGRESSION_PASS`) прогоняется на каждом мердж-коммите
   пакета**, а не однократно. В режиме §11.1 мердж-коммитов столько же, сколько
   пакетов, и каждый из них — отдельная возможность задеть AI-ядро. Это
   единственное, что режим §11.1 добавляет к работе по сравнению с §11.2, и это
   цена за отсутствие события «большой мердж».
4. **Пакет, не прошедший свои gates, в `release` не вливается вообще.** Правило
   `TZ_CODEX_V2` §2.5 о мердже только зелёным действует и здесь; в режиме §11.1
   оно строже по последствиям, потому что вливается сразу в продуктовую линию.
