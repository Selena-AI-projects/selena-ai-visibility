...
export type LocalOperatorAlertCode = "RAW_RETENTION_OVERDUE" | "NOTIFICATION_TEST";
...
export function localOperatorAlertContent(alert: Alert, appUrl: string) {
	...
	const subject = `[Selena] Local Visibility: ${alert.code}`;
	const text = `${subject}\nUTC: ${time}\n${messages[alert.code]}\n${url}\nDo not enable paid provider execution to resolve this alert.`;
	return {
		subject,
		text,
		html: `<p>${messages[alert.code]}</p><p>UTC: ${time}</p><p><a href="${url}">Open Selena</a></p><p>Paid provider execution remains disabled.</p>`,
	};
}
...
export async function registerLocalOperatorAlerts(
	...
	if (env.SELENA_LOCAL_OPERATOR_ALERTS_ENABLED !== "true" || !recipient || !appUrl) return undefined;
	...
	try {
		...
		await boss.work<Alert>(LOCAL_OPERATOR_ALERT_QUEUE, async (jobs) => {
			for (const job of jobs) {
				try {
					const receipt = await send(recipient, localOperatorAlertContent(job.data, appUrl), `local-alert-${job.id}`);
	...
	return async (code: LocalOperatorAlertCode) => {
		...
		for (let attempt = 0; attempt < 3; attempt++) {
			try {
				const receipt = await send(recipient, localOperatorAlertContent(alert, appUrl), `local-alert-${id}`);
