...
## D-045-RECONCILIATION — Do not adopt the uncommitted coordinate-proof diff unchanged
...
- Effect: this was an interim hold only. It was superseded by D-046 after the proof-aware snapshot, hash-compatibility regression, route/repository integration and duplicate-proof policy were implemented and committed.

## D-046 — Store Local AI coordinate proof as distinct evidence
...
- Evidence: source commits `f9bc622b` and `1a3900aa`; focused Node 24 contracts `39/39`, web API/read tests `49/49`, manual-pilot tests `7/7`, contracts/lib/web typechecks, targeted Biome and `git diff --check` pass. The broader suite/build evidence from the preceding source slice remains recorded separately; no migration, database, provider, credential or network action was used.
...
## D-047 — Fail closed on Local AI evidence identity drift

- Decision: treat screenshot and coordinate proof as distinct only when both their trimmed private references and canonical SHA-256 digests differ; canonical digest comparison strips an optional `sha256:` prefix. Count every `COORDINATE_PROOF` row before validating usability, so a malformed duplicate cannot be ignored. Preserve the compatibility `contextHash`, but bind the stronger hash-plus-`pointId` task identity independently back to a Lock observer context. Use server-created evidence timestamps, not caller-controlled capture times, for collection mutation high-water.
- Evidence: source-only commit `d2afce4f`; two independent blind audits found the alias, malformed-duplicate, point-drift and future-timestamp gaps, and a read-only fix verification confirmed all four closures after catching and retesting the digest-prefix bypass. Node 24 contracts `231/231`, web `372/372` plus 4 skipped, lib `891/891`, worker bootstrap `3/3`, contracts/lib/web/worker typechecks, targeted Biome, web build and worker build pass. The first concurrent lib run hit the known 5-second 125-slot timing test; its isolated `5/5` rerun and the subsequent standalone full lib `891/891` run passed without changing the timeout.
- Effect: incomplete, aliased, duplicated or Lock-drifted coordinate evidence remains `UNKNOWN` or is rejected before persistence, and a client-supplied future capture time cannot freeze evidence cursor invalidation. This narrows D-022: evidence ordering may still use captured time, but snapshot mutation identity uses server-created time. Object-byte verification, database constraints and runtime proof remain owner-gated/`UNKNOWN`; no migration, database, provider, credential, network, staging or production action was used.

## D-048 — Make manual Local AI review atomic and single-transition

- Decision: review may transition only an observation/task pair currently in `SUBMITTED_FOR_REVIEW`. Lock both tenant-owned rows, apply both conditional updates, and write the audit receipt in one transaction. Replay is allowed only for the same observation, idempotency key, decision and reason; a changed payload conflicts, and a different key cannot rewrite a terminal decision.
- Evidence: source-only commit `85b91906`; behavioral tests cover exact replay, changed decision, changed reason, key isolation, cross-tenant access, terminal-state rejection and rollback after an audit failure. Node 24 lib `898/898`, lib typecheck, targeted Biome and `git diff --check` pass; the independent fix verifier returned `PASS` after its test-model findings were closed.
- Effect: a partial failure or concurrent reviewer cannot leave observation, task and audit in different states or rewrite accepted/invalid evidence in place. Correction/supersession semantics remain a separate owner/product decision; no migration, database, provider, credential, network, staging or production action was used.

## D-049 — Hold migration rehearsal on completion-audit contract conflicts

- Decision: do not request or run the disposable migration rehearsal while the source store and migration 0044 disagree on lease mutation, and do not silently version Maps keyword or Local AI point identity contracts. These require explicit owner choices and forward-compatible repair plans.
- Evidence: three independent read-only completion audits of immutable snapshot `5942c336` converged on fail-closed but incomplete runtime wiring and found the lease-transition conflict, mutable keyword request input, point-aware task collision, aggregate-budget/idempotency gaps and missing runtime RLS proof. No database, provider, staging or paid action was run.
- Effect: current defaults remain safe and inactive, but prior static `0044 PASS` wording is superseded by `CONFLICT`. The next owner packet must choose the migration/runtime repair and identity-versioning direction before disposable PostgreSQL evidence is meaningful.
