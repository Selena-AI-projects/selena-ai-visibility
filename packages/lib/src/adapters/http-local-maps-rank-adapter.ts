...
type HttpLocalMapsResponse = {
	...
	event: {
		kind: "FOUND" | "ABSENT_WITHIN_DEPTH" | "RETRYABLE_FAILURE" | "PROVIDER_AUTH_FAILURE" | "OUTCOME_UNKNOWN";
		reason?: string;
	};
 ...
 */
export function createHttpLocalMapsRankAdapter(
	env: Record<string, string | undefined> = process.env,
): LocalMapsRankAdapter<HttpLocalMapsResponse> {
	...
	return {
		...
		async execute(
			task: LocalMapsMaterializedProviderRequest,
			permit: LocalMapsRankPermit,
		): Promise<HttpLocalMapsResponse> {
