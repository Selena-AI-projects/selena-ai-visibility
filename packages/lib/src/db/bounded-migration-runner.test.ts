...
describe("bounded migration runner", () => {
	it("builds an exact Drizzle bundle through the reviewed hold release", () => {
		...
		expect(journal.entries.at(-1)).toMatchObject({ idx: 56, tag: "0056_journal_daily_claim_hold_release" });
		expect(readdirSync(targetDirectory)).toContain("0056_journal_daily_claim_hold_release.sql");
