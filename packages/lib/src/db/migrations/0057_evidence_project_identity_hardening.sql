...
BEGIN
	...
	END IF;
	IF EXISTS (SELECT 1 FROM "sv_evidence_acceptance_receipts")
		OR EXISTS (
			SELECT 1
			FROM "sv_audit_events"
			WHERE "event" = 'PROVIDER_EVIDENCE_FORMALLY_ACCEPTED'
		) THEN
		RAISE EXCEPTION 'EVIDENCE_PROJECT_IDENTITY_LEGACY_ACCEPTANCE_REVIEW_REQUIRED';
	END IF;
END;
$$;
