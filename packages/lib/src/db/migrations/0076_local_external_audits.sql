-- Local external audits: retained provider responses imported by an operator,
-- kept apart from application attempts and cost events. Staging carries these
-- from its own 0073 and 0075; the body is those files verbatim, applied only
-- where the objects are absent, and the check afterwards rejects a partial copy.
DO $local_external$
BEGIN
IF to_regclass('public.sv_local_external_audits') IS NULL THEN

-- ==== staging 0073_local_external_audits.sql ====
...
-- ==== staging 0075_local_external_legacy_identity.sql ====
...
DO $local_external_verify$
DECLARE missing text;
BEGIN
 SELECT string_agg(name, ', ') INTO missing FROM (
  SELECT t AS name FROM unnest(ARRAY['sv_local_external_audits', 'sv_local_external_tasks', 'sv_local_external_raw_evidence', 'sv_local_external_publications']) t
   WHERE to_regclass('public.'||t) IS NULL
  UNION ALL
  SELECT f FROM unnest(ARRAY['sv_local_external_immutable', 'sv_local_external_register_tasks', 'sv_local_external_task_guard', 'sv_local_external_raw_guard', 'sv_local_external_publication_guard']) f
   WHERE to_regprocedure('public.'||f||'()') IS NULL
  UNION ALL
  SELECT 'sv_local_external_publication_guard accepts CID-only locks'
   WHERE position($q$l.snapshot#>>'{targetIdentity,placeId}' IS NULL$q$ IN
    coalesce((SELECT prosrc FROM pg_proc WHERE oid=to_regprocedure('public.sv_local_external_publication_guard()')),''))=0
 ) m;
 IF missing IS NOT NULL THEN RAISE EXCEPTION 'LOCAL_EXTERNAL_SCHEMA_INCOMPLETE: %', missing; END IF;
END; $local_external_verify$;
ALTER TABLE sv_local_external_audits FORCE ROW LEVEL SECURITY;
ALTER TABLE sv_local_external_tasks FORCE ROW LEVEL SECURITY;
ALTER TABLE sv_local_external_raw_evidence FORCE ROW LEVEL SECURITY;
ALTER TABLE sv_local_external_publications FORCE ROW LEVEL SECURITY;
