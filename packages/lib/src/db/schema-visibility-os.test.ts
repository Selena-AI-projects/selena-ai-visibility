...
describe("Visibility OS provider evidence provenance", () => {
	...
	it("keeps the post-0056 runtime role bootstrap idempotent and explicitly allowlisted", () => {
		...
		expect(roleBootstrap).toContain("SELENA_RUNTIME_ROLE_REQUIRES_MIGRATION_0071");
...
describe("Visibility OS Outcome Layer schema", () => {
	it("registers M2 through formal evidence hardening as one ordered numbered migration chain", () => {
		...
		expect(journal.entries.slice(-34)).toEqual([
			...
			{ idx: 71, version: "7", when: 1790320003000, tag: "0071_identifier_availability", breakpoints: true },
			{ idx: 72, version: "7", when: 1790320004000, tag: "0072_answer_retention_sweep", breakpoints: true },
			{ idx: 73, version: "7", when: 1790320005000, tag: "0073_operator_access_audit", breakpoints: true },
		]);
	});
