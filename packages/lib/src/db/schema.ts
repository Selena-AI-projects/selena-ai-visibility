...
export const svJournalDailyClaims = pgTable(
	...
	(table) => ({
		...
		statusCheck: check(
			...
			sql`${table.status} IN ('CLAIMED', 'EXECUTING', 'NO_SPEND', 'HOLD', 'COMPLETED', 'ABANDONED')`,
		),
		utcDayCheck: check(
		...
		completionCheck: check(
			...
			sql`((${table.status} IN ('CLAIMED', 'EXECUTING', 'NO_SPEND', 'HOLD') AND ${table.completedAt} IS NULL AND ${table.abandonedAt} IS NULL) OR (${table.status} = 'COMPLETED' AND ${table.completedAt} IS NOT NULL AND ${table.completedAt} >= ${table.claimedAt} AND ${table.abandonedAt} IS NULL) OR (${table.status} = 'ABANDONED' AND ${table.completedAt} IS NULL AND ${table.abandonedAt} IS NOT NULL AND ${table.abandonedAt} >= ${table.claimedAt}))`,
		),
	}),
).enableRLS();

/**
 * A committed pre-transport fence for a journal permit. Presence means provider
 * spend is possible, never that a response or a charge was observed.
 */
