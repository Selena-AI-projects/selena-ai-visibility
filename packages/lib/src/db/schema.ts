...
export const svApiIdempotencyRecords = pgTable(
	"sv_api_idempotency_records",
	{
		id: uuid("id").defaultRandom().primaryKey().notNull(),
		organizationId: text("organization_id")
			.notNull()
	...
	(table) => ({
		identityUnique: uniqueIndex("sv_api_idempotency_identity_unique").on(
		...
		keyLengthCheck: check(
			"sv_api_idempotency_key_length_check",
			sql`length(${table.idempotencyKey}) BETWEEN 8 AND 128 AND ${table.idempotencyKey} = btrim(${table.idempotencyKey})`,
		),
		responseStatusCheck: check(
			"sv_api_idempotency_response_status_check",
			sql`${table.responseStatus} BETWEEN 200 AND 299`,
		),
		expiryCheck: check(
			"sv_api_idempotency_expiry_check",
...
export const svConfigurationLocks = pgTable(
	...
	(table) => ({
		projectVersionUnique: uniqueIndex("sv_locks_project_version_unique").on(
			table.projectId,
			table.version,
			table.legacyCollisionOrdinal,
		),
		idOrganizationUnique: uniqueIndex("sv_configuration_locks_id_organization_unique").on(
			table.id,
			table.organizationId,
		),
		idProjectOrganizationUnique: uniqueIndex("sv_configuration_locks_id_project_org_unique").on(
			table.id,
			table.projectId,
			table.organizationId,
		),
		...
		legacyCollisionOrdinalCheck: check(
			"sv_configuration_locks_legacy_collision_ordinal_check",
			sql`${table.legacyCollisionOrdinal} >= 0`,
		),
	}),
).enableRLS();

/** Durable fail-closed spend serialization for the manually-invoked owner journal. */
export const svJournalDailyClaims = pgTable(
	"sv_journal_daily_claims",
	{
		id: uuid("id").defaultRandom().primaryKey().notNull(),
		organizationId: text("organization_id")
			.notNull()
	...
	(table) => ({
		identityUnique: uniqueIndex("sv_journal_daily_claims_identity_unique").on(
			table.organizationId,
			table.projectId,
			table.questionSetVersion,
			table.utcDay,
			table.attempt,
		),
		unresolvedUnique: uniqueIndex("sv_journal_daily_claims_unresolved_unique")
			.on(table.organizationId, table.projectId)
			.where(sql`${table.status} IN ('CLAIMED', 'EXECUTING', 'HOLD')`),
		lockUnique: uniqueIndex("sv_journal_daily_claims_lock_unique")
			.on(table.configurationLockId)
			.where(sql`${table.configurationLockId} IS NOT NULL`),
		projectOrganizationReference: foreignKey({
			columns: [table.projectId, table.organizationId],
			foreignColumns: [svProjects.id, svProjects.organizationId],
		...
		lockProjectOrganizationReference: foreignKey({
			columns: [table.configurationLockId, table.projectId, table.organizationId],
			foreignColumns: [svConfigurationLocks.id, svConfigurationLocks.projectId, svConfigurationLocks.organizationId],
		...
		statusCheck: check(
			"sv_journal_daily_claims_status_check",
			sql`${table.status} IN ('CLAIMED', 'EXECUTING', 'NO_SPEND', 'HOLD', 'COMPLETED', 'ABANDONED', 'RECONCILED')`,
		),
		utcDayCheck: check(
			"sv_journal_daily_claims_utc_day_check",
			sql`${table.utcDay} = (${table.claimedAt} AT TIME ZONE 'UTC')::date`,
		),
		executionLinkCheck: check(
			"sv_journal_daily_claims_execution_link_check",
			sql`${table.status} IN ('CLAIMED', 'NO_SPEND', 'ABANDONED') OR ${table.configurationLockId} IS NOT NULL`,
		),
		completionCheck: check(
			"sv_journal_daily_claims_completion_check",
...
export const svCycles = pgTable(
	...
	(table) => ({
		...
		idOrganizationUnique: uniqueIndex("sv_cycles_id_organization_unique").on(table.id, table.organizationId),
	}),
).enableRLS();
...
export const svJournalProviderBoundaries = pgTable(
	"sv_journal_provider_boundaries",
	{
		id: uuid("id").defaultRandom().primaryKey().notNull(),
		organizationId: text("organization_id")
			.notNull()
			.references(() => organization.id),
		projectId: uuid("project_id").notNull(),
		journalClaimId: uuid("journal_claim_id")
			.notNull()
			.references(() => svJournalDailyClaims.id),
	...
	(table) => ({
		claimPermitUnique: uniqueIndex("sv_journal_provider_boundaries_claim_permit_unique").on(
			table.journalClaimId,
			table.permitId,
		),
		runUnique: uniqueIndex("sv_journal_provider_boundaries_run_unique").on(table.runId),
		organizationClaimIdx: index("sv_journal_provider_boundaries_org_claim_idx").on(
			table.organizationId,
			table.journalClaimId,
		),
		projectOrganizationReference: foreignKey({
		...
		providerCallUpperBoundCheck: check(
			"sv_journal_provider_boundaries_call_bound_check",
			sql`${table.providerCallUpperBound} = 1`,
		),
	}),
).enableRLS();
...
export const svResponseMentions = pgTable(
	"sv_response_mentions",
	{
		id: uuid("id").defaultRandom().primaryKey().notNull(),
		organizationId: text("organization_id")
			.notNull()
...
export const svCitationGapSnapshots = pgTable(
	"sv_citation_gap_snapshots",
	{
		id: uuid("id").defaultRandom().primaryKey().notNull(),
		organizationId: text("organization_id")
			.notNull()
		...
		repeatStability: numeric("repeat_stability", { precision: 6, scale: 4 }),
	...
	(table) => ({
		cycleSourceUnique: uniqueIndex("sv_citation_gap_cycle_source_unique").on(
			table.cycleId,
			table.sourceDomain,
...
export const svCostEvents = pgTable(
	...
	(table) => ({
		idOrganizationCycleDomainUnique: uniqueIndex("sv_cost_events_id_org_cycle_domain_unique").on(
			table.id,
			table.organizationId,
		...
		domainShapeCheck: check(
			"sv_cost_events_domain_shape_check",
			sql`${table.domainId} = 'AI' OR (${table.measurementCycleId} IS NOT NULL AND ${table.cycleId} IS NULL AND ${table.runId} IS NULL)`,
		),
	}),
).enableRLS();
...
export const svOrderRequests = pgTable(
	"sv_order_requests",
	{
		id: uuid("id").defaultRandom().primaryKey().notNull(),
		organizationId: text("organization_id")
	...
	(table) => ({ orgCreatedIdx: index("sv_order_requests_org_created_idx").on(table.organizationId, table.createdAt) }),
).enableRLS();
...
export const svPilotInvites = pgTable(
	"sv_pilot_invites",
	{
		id: uuid("id").defaultRandom().primaryKey().notNull(),
	...
	(table) => ({ codeHashKey: unique("sv_pilot_invites_code_hash_key").on(table.codeHash) }),
).enableRLS();
...
export const svFreeAutoDispatchClaims = pgTable(
	"sv_free_auto_dispatch_claims",
	{
		id: uuid("id").defaultRandom().primaryKey().notNull(),
...
export const svSimulationSubscriptions = pgTable(
	"sv_simulation_subscriptions",
	{
		id: uuid("id").defaultRandom().primaryKey().notNull(),
		organizationId: text("organization_id")
			.notNull()
	...
	(table) => ({
		eventUnique: uniqueIndex("sv_simulation_subscriptions_event_unique").on(
			table.organizationId,
			table.provider,
		...
		notAMeasurementCheck: check(
			"sv_simulation_subscriptions_not_a_measurement_check",
			sql`${table.notAMeasurement} = true`,
		),
		statusCheck: check(
			"sv_simulation_subscriptions_status_check",
...
export const svSimulationConnectTokens = pgTable(
	"sv_simulation_connect_tokens",
	{
		id: uuid("id").defaultRandom().primaryKey().notNull(),
		organizationId: text("organization_id")
			.notNull()
...
export const svSimulationRecipients = pgTable(
	"sv_simulation_recipients",
	{
		id: uuid("id").defaultRandom().primaryKey().notNull(),
		organizationId: text("organization_id")
			.notNull()
	...
	(table) => ({
		activeUnique: uniqueIndex("sv_simulation_recipients_active_unique")
			.on(table.organizationId, table.projectRef, table.channel)
		...
		statusCheck: check(
			"sv_simulation_recipients_status_check",
			sql`${table.status} IN ('PENDING', 'BOUND', 'UNBOUND')`,
		),
		unboundCheck: check(
			"sv_simulation_recipients_unbound_check",
			sql`(${table.status} = 'UNBOUND') = (${table.unboundAt} IS NOT NULL)`,
		),
	}),
).enableRLS();
...
export const svSimulationReports = pgTable(
	"sv_simulation_reports",
	{
		id: uuid("id").defaultRandom().primaryKey().notNull(),
		organizationId: text("organization_id")
			.notNull()
		...
		subscriptionId: uuid("subscription_id")
			.notNull()
			.references(() => svSimulationSubscriptions.id),
	...
	(table) => ({
		periodUnique: uniqueIndex("sv_simulation_reports_period_unique").on(
			table.organizationId,
			table.projectRef,
			table.periodStart,
		),
...
export const svSimulationDeliveries = pgTable(
	"sv_simulation_deliveries",
	{
		id: uuid("id").defaultRandom().primaryKey().notNull(),
		organizationId: text("organization_id")
			.notNull()
		...
		reportId: uuid("report_id")
			.notNull()
			.references(() => svSimulationReports.id),
		recipientId: uuid("recipient_id")
			.notNull()
			.references(() => svSimulationRecipients.id),
	...
	(table) => ({
		...
		statusCheck: check(
			"sv_simulation_deliveries_status_check",
			sql`${table.status} IN ('PENDING', 'SENDING', 'DELIVERED', 'RETRY_SCHEDULED', 'FAILED', 'UNBOUND')`,
		),
		...
		deliveredCheck: check(
			"sv_simulation_deliveries_delivered_check",
			sql`(${table.status} = 'DELIVERED') = (${table.deliveredAt} IS NOT NULL)`,
		),
		claimedCheck: check(
			"sv_simulation_deliveries_claimed_check",
...
export const svSimulationBootstrapNonces = pgTable(
	"sv_simulation_bootstrap_nonces",
	{
...
export const svSimulationDeliveryAttempts = pgTable(
	"sv_simulation_delivery_attempts",
	{
		id: uuid("id").defaultRandom().primaryKey().notNull(),
		organizationId: text("organization_id")
			.notNull()
		...
		deliveryId: uuid("delivery_id")
			.notNull()
			.references(() => svSimulationDeliveries.id),
		attempt: integer("attempt").notNull(),
	...
	(table) => ({
		...
		outcomeCheck: check(
			"sv_simulation_delivery_attempts_outcome_check",
			sql`${table.outcome} IN ('SUCCESS', 'TEMPORARY_FAILURE', 'RECIPIENT_GONE')`,
		),
	}),
).enableRLS();

/** A single-use link that binds a Telegram chat to one project's weekly digest. Only its hash is stored. */
export const svDeliveryConnectTokens = pgTable(
	"sv_delivery_connect_tokens",
	{
		id: uuid("id").defaultRandom().primaryKey().notNull(),
		organizationId: text("organization_id")
			.notNull()
	...
	(table) => ({
		projectReference: foreignKey({
			columns: [table.projectId, table.organizationId],
			foreignColumns: [svProjects.id, svProjects.organizationId],
			name: "sv_delivery_connect_tokens_project_fk",
		}),
...
export const svDeliveryRecipients = pgTable(
	"sv_delivery_recipients",
	{
		id: uuid("id").defaultRandom().primaryKey().notNull(),
		organizationId: text("organization_id")
			.notNull()
	...
	(table) => ({
		projectReference: foreignKey({
			columns: [table.projectId, table.organizationId],
			foreignColumns: [svProjects.id, svProjects.organizationId],
			name: "sv_delivery_recipients_project_fk",
		}),
		idOrganizationUnique: uniqueIndex("sv_delivery_recipients_id_organization_unique").on(
			table.id,
			table.organizationId,
		),
		activeUnique: uniqueIndex("sv_delivery_recipients_active_unique")
			.on(table.organizationId, table.projectId, table.channel)
			.where(sql`${table.status} = 'BOUND'`),
		channelCheck: check("sv_delivery_recipients_channel_check", sql`${table.channel} = 'telegram'`),
		localeCheck: check("sv_delivery_recipients_locale_check", sql`${table.locale} IN ('ru', 'en')`),
		statusCheck: check("sv_delivery_recipients_status_check", sql`${table.status} IN ('BOUND', 'UNBOUND')`),
		unboundCheck: check(
			"sv_delivery_recipients_unbound_check",
			sql`(${table.status} = 'UNBOUND') = (${table.unboundAt} IS NOT NULL)`,
		),
	}),
).enableRLS();

/** What was sent for one project and week, saved before sending and immutable afterwards. */
export const svWeeklyDigests = pgTable(
	"sv_weekly_digests",
	{
		id: uuid("id").defaultRandom().primaryKey().notNull(),
		organizationId: text("organization_id")
	...
	(table) => ({
		projectReference: foreignKey({
			columns: [table.projectId, table.organizationId],
			foreignColumns: [svProjects.id, svProjects.organizationId],
			name: "sv_weekly_digests_project_fk",
		}),
		cycleReference: foreignKey({
			columns: [table.cycleId, table.organizationId],
			foreignColumns: [svCycles.id, svCycles.organizationId],
			name: "sv_weekly_digests_cycle_fk",
		}),
		previousCycleReference: foreignKey({
			columns: [table.previousCycleId, table.organizationId],
			foreignColumns: [svCycles.id, svCycles.organizationId],
			name: "sv_weekly_digests_previous_cycle_fk",
		}),
		idOrganizationUnique: uniqueIndex("sv_weekly_digests_id_organization_unique").on(table.id, table.organizationId),
		periodUnique: uniqueIndex("sv_weekly_digests_period_unique").on(
			table.organizationId,
			table.projectId,
			table.periodStart,
		),
		periodCheck: check("sv_weekly_digests_period_check", sql`${table.periodEnd} > ${table.periodStart}`),
		cyclesCheck: check(
			"sv_weekly_digests_cycles_check",
			sql`${table.previousCycleId} IS DISTINCT FROM ${table.cycleId}`,
		),
	}),
).enableRLS();

export const svDigestDeliveries = pgTable(
	"sv_digest_deliveries",
	{
		id: uuid("id").defaultRandom().primaryKey().notNull(),
		organizationId: text("organization_id")
			.notNull()
	...
	(table) => ({
		digestReference: foreignKey({
			columns: [table.digestId, table.organizationId],
			foreignColumns: [svWeeklyDigests.id, svWeeklyDigests.organizationId],
			name: "sv_digest_deliveries_digest_fk",
		}),
		recipientReference: foreignKey({
			columns: [table.recipientId, table.organizationId],
			foreignColumns: [svDeliveryRecipients.id, svDeliveryRecipients.organizationId],
			name: "sv_digest_deliveries_recipient_fk",
		}),
		idOrganizationUnique: uniqueIndex("sv_digest_deliveries_id_organization_unique").on(table.id, table.organizationId),
		digestRecipientUnique: uniqueIndex("sv_digest_deliveries_digest_recipient_unique").on(
			table.digestId,
			table.recipientId,
		),
		dueIdx: index("sv_digest_deliveries_due_idx").on(table.status, table.nextAttemptAt),
		statusCheck: check(
			"sv_digest_deliveries_status_check",
			sql`${table.status} IN ('PENDING', 'SENDING', 'DELIVERED', 'RETRY_SCHEDULED', 'FAILED', 'UNBOUND')`,
		),
		attemptCapCheck: check("sv_digest_deliveries_attempt_cap_check", sql`${table.attemptsMade} BETWEEN 0 AND 5`),
		deliveredCheck: check(
			"sv_digest_deliveries_delivered_check",
			sql`(${table.status} = 'DELIVERED') = (${table.deliveredAt} IS NOT NULL)`,
		),
		claimedCheck: check(
			"sv_digest_deliveries_claimed_check",
			sql`${table.status} <> 'SENDING' OR ${table.claimedAt} IS NOT NULL`,
		),
	}),
).enableRLS();

export const svDigestDeliveryAttempts = pgTable(
	"sv_digest_delivery_attempts",
	{
		id: uuid("id").defaultRandom().primaryKey().notNull(),
		organizationId: text("organization_id")
			.notNull()
	...
	(table) => ({
		deliveryReference: foreignKey({
			columns: [table.deliveryId, table.organizationId],
			foreignColumns: [svDigestDeliveries.id, svDigestDeliveries.organizationId],
			name: "sv_digest_delivery_attempts_delivery_fk",
		}),
		attemptUnique: uniqueIndex("sv_digest_delivery_attempts_unique").on(table.deliveryId, table.attempt),
		rangeCheck: check("sv_digest_delivery_attempts_range_check", sql`${table.attempt} BETWEEN 1 AND 5`),
		outcomeCheck: check(
			"sv_digest_delivery_attempts_outcome_check",
			sql`${table.outcome} IN ('SUCCESS', 'TEMPORARY_FAILURE', 'RECIPIENT_GONE')`,
		),
	}),
).enableRLS();
