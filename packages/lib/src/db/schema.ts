...
}, (table) => ({ dispatchUnique: uniqueIndex("sv_run_permits_dispatch_key_unique").on(table.dispatchKey), orgCycleIdx: index("sv_run_permits_org_cycle_idx").on(table.organizationId, table.cycleId), statusCheck: check("sv_run_permits_status_check", sql`${table.status} IN ('issued', 'consumed', 'revoked', 'cancelled')`) })).enableRLS();
...
export const svJournalProviderBoundaries = pgTable("sv_journal_provider_boundaries", {
	id: uuid("id").defaultRandom().primaryKey().notNull(), organizationId: text("organization_id").notNull().references(() => organization.id), projectId: uuid("project_id").notNull(), journalClaimId: uuid("journal_claim_id").notNull().references(() => svJournalDailyClaims.id), configurationLockId: uuid("configuration_lock_id").notNull(), cycleId: uuid("cycle_id").notNull().references(() => svCycles.id), permitId: uuid("permit_id").notNull().references(() => svRunPermits.id), runId: uuid("run_id").notNull().references(() => svRuns.id), dispatchKey: text("dispatch_key").notNull(), channel: text("channel").notNull(), systemId: text("system_id"), boundaryKind: text("boundary_kind").notNull().default("PRE_TRANSPORT"), providerCallUpperBound: integer("provider_call_upper_bound").notNull().default(1), crossedAt: timestamp("crossed_at", { withTimezone: true }).default(sql`clock_timestamp()`).notNull(),
}, (table) => ({ claimPermitUnique: uniqueIndex("sv_journal_provider_boundaries_claim_permit_unique").on(table.journalClaimId, table.permitId), runUnique: uniqueIndex("sv_journal_provider_boundaries_run_unique").on(table.runId), organizationClaimIdx: index("sv_journal_provider_boundaries_org_claim_idx").on(table.organizationId, table.journalClaimId), projectOrganizationReference: foreignKey({ columns: [table.projectId, table.organizationId], foreignColumns: [svProjects.id, svProjects.organizationId], name: "sv_journal_provider_boundaries_project_org_fk" }), lockProjectOrganizationReference: foreignKey({ columns: [table.configurationLockId, table.projectId, table.organizationId], foreignColumns: [svConfigurationLocks.id, svConfigurationLocks.projectId, svConfigurationLocks.organizationId], name: "sv_journal_provider_boundaries_lock_project_org_fk" }), boundaryKindCheck: check("sv_journal_provider_boundaries_kind_check", sql`${table.boundaryKind} = 'PRE_TRANSPORT'`), providerCallUpperBoundCheck: check("sv_journal_provider_boundaries_call_bound_check", sql`${table.providerCallUpperBound} = 1`) })).enableRLS();
...
}, (table) => ({ idOrganizationCycleDomainUnique: uniqueIndex("sv_cost_events_id_org_cycle_domain_unique").on(table.id, table.organizationId, table.measurementCycleId, table.domainId), orgCycleIdx: index("sv_cost_events_org_cycle_idx").on(table.organizationId, table.cycleId), orgDomainIdx: index("sv_cost_events_org_domain_idx").on(table.organizationId, table.domainId), measurementCycleIdx: index("sv_cost_events_measurement_cycle_idx").on(table.measurementCycleId), runIdx: index("sv_cost_events_run_idx").on(table.runId), domainShapeCheck: check("sv_cost_events_domain_shape_check", sql`${table.domainId} = 'AI' OR (${table.measurementCycleId} IS NOT NULL AND ${table.cycleId} IS NULL AND ${table.runId} IS NULL)`) })).enableRLS();
