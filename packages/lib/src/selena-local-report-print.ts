import type { LocalCompetitor } from "./selena-local-competitors";
import type { ExternalLocalAuditContent } from "./selena-local-external-audit";
import type { LocalReportContent } from "./selena-local-report-publication";
...
type PrintPoint = {
	...
	organic: LocalCompetitor[];
	ads: LocalCompetitor[];
	comparisonAvailable: boolean;
};
export function localReportPrintableHtml(
	report: LocalReportContent,
	external: readonly ExternalLocalAuditContent[] = [],
	requestParameters?: { depth?: number; device?: string; zoom?: number; seDomain?: string },
) {
	const groups: Array<{ keyword: string; language: string; source: string; points: PrintPoint[] }> = [];
				...
				organic:
					row.competition?.status === "AVAILABLE"
						? row.competition.target
							? row.competition.aboveTarget
							: (row.competition.returnedOrganic ?? [])
						: [],
	...
	for (const audit of external)
		for (const batch of audit.batches)
			for (const keyword of [...new Set(batch.observations.map((r) => r.keyword))]) {
	...
	const target =
		report.observations
			.map((r) => (r.competition?.status === "AVAILABLE" ? r.competition.target?.name : null))
			.find(Boolean) ?? "Restaurant";
	const list = (items: LocalCompetitor[]) =>
		items
			.slice(0, 3)
			.map(
				(item) =>
					`${escapeHtml(item.name)} (${item.kind === "AD" ? `overall #${item.absoluteRank ?? "unknown"}` : `organic #${item.groupRank}`})`,
			)
			.join("<br>") + (items.length > 3 ? "<br>Full list in CSV" : "");
	const sections = groups
		.map((g) => {
			const valid = g.points.filter((r) => ["FOUND", "ABSENT_WITHIN_DEPTH"].includes(r.outcome));
			const found = valid.filter((r) => r.rank !== null).length,
				top3 = valid.filter((r) => r.rank !== null && r.rank <= 3).length;
