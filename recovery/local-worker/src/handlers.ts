...
import { type SelenaLocalRawRetentionData, selenaLocalRawRetentionJob } from "./jobs/selena-local-raw-retention";
...
export async function registerHandlers(boss: PgBoss): Promise<void> {
	...
	await boss.work<SelenaLocalRawRetentionData>(
		"selena-local-raw-retention",
		{ localConcurrency: 1 },
		withSentry("selena-local-raw-retention", selenaLocalRawRetentionJob),
	);
	console.log("Registered handler: selena-local-raw-retention");
