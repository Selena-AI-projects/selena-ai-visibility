...
1. **Подключения.** Web использует `SELENA_WEB_DATABASE_URL` (роль `selena_app`), воркер — `SELENA_WORKER_DATABASE_URL`, `migrate` — `DATABASE_URL` (owner).
   - Какой сервис что использует, определяет переменная `SELENA_DATABASE_SURFACE` (`web` / `worker` / `migrate`).
   - Правило «не стартовать без переменной» нельзя привязывать к `DEPLOYMENT_MODE`: staging и production Selena работают с `DEPLOYMENT_MODE=local`, и такое правило никогда бы не сработало. Условие: запуск на хостинге. Какой признак Railway надёжно есть у каждого сервиса, нужно проверить. Если ни один не подходит, вместо него используется явная `SELENA_HOSTED=true`.
   - Прямой `drizzle(DATABASE_URL)` в `db.ts` убрать: все пулы (`db`, `postgres-read`, pg-boss, `admin.ts`, better-auth) строятся через один резолвер.
   - При старте проверить, что у текущей роли нет `rolbypassrls` и `rolsuper` и она не владеет ни одной таблицей.
...
5. **Контекст запроса.** Любой запрос `selena_app` к данным выполняется внутри `withOrganizationTransaction`, которая ставит `app.organization_id` и `app.user_id` (сейчас ставится только `app.organization_id`). Сейчас прямой `db.` есть в 19 API-обработчиках и 60 server functions, включая общий поиск членства `resolveSessionAuthContext`; список лежит в `ACCESS_REGISTRY.md`. Для bootstrap-поиска членства и brand→org нужна `SECURITY DEFINER` функция. Web также пишет напрямую в `pgboss.job`, поэтому нужны отдельные права. Добавить линтер или проверку в тестах, запрещающую прямой `db.` вне этой обёртки в `apps/web/src/server`.
...
8. **Находки реестра маршрутов, обязательные к исправлению при переносе:**
   - Маршруты `admin/local-scan-cycles/*` (QC, publish, deliver) сейчас пропускают любого владельца организации, то есть клиент сам проводит QC и публикует свой отчёт. Нужна отдельная проверка оператора платформы (`selena_internal`).
   - Тестовая оплата `$orderId/test-payment` (SUCCEEDED, $49) открывает публикацию через `sv_publish_local_order`. Этот маршрут не переносить. Условие публикации переписать без привязки к LEGACY-оплате.
   - Защита от CSRF: все POST требуют точного совпадения `Origin`, без исключений.
   - `resolveSessionAuthContext`: если активная организация не выбрана, сейчас берётся `rows[0]` без `ORDER BY`. Нужно либо сделать выбор детерминированным, либо требовать явный выбор организации.
   - Промышленные флаги вместо проверки ID окружения Railway: жёстко выключены в production, включаются только явно.
   - Для чужого объекта отвечать 404, а не 400.
   - `/api/local-ready` не должен запускать запросы при каждом обращении: нужен кэш или отдельный внутренний healthcheck.
9. **Скрипт `selena-rls-runtime-role.sql`** превратить в воспроизводимый шаг деплоя. Скрипт должен быть идемпотентным, пароль передаётся из секрета, права — по allowlist, который генерируется и проверяется тестом.
...
3. **M2-a:** реестр доступа (все маршруты плюс 21 невосстановленный), решения по 16 таблицам без политик, тестовый стенд с четырьмя identity. Только тесты и документы, без изменения поведения.
...
21 серверный маршрут существует только в собранном JS. Из них 15 требуют малых усилий, 5 — больших, 2 не переносятся (`/api/selena-report-document` с данными клиента и `test-payment`), 6 связаны с LEGACY-предложением $49. Реестр со строками маршрут → назначение → вызывающие → доказательства в бандле → требования к авторизации → затрагиваемые данные → нужно ли переписывать → куда → приёмочный тест лежит в `recovery/m1a-m2/ROUTE_REGISTRY.md`. Ни один маршрут не переносится без строки в реестре и приёмочного теста.
