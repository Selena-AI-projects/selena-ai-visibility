...
export const localApiMapResultStatuses = ["FOUND", "ABSENT_WITHIN_DEPTH", "INVALID", "UNKNOWN", "BLOCKED", "PENDING", "CANCELLED"] as const;
...
export const localApiMapResultSchema = z
	...
	.superRefine((result, context) => {
		if (result.status === "PENDING" && (result.reasonCode !== null || result.capturedAt !== null || result.evidenceIds.length !== 0))
			context.addIssue({ code: "custom", message: "LOCAL_API_PENDING_EVIDENCE_FORBIDDEN" });
		if (result.status === "CANCELLED" && (result.reasonCode !== "LOCAL_STOPPED" || result.capturedAt !== null || result.evidenceIds.length !== 0))
