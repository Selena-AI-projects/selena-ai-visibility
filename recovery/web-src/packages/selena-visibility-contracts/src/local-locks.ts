...
export const mapsLockV1Schema = z
	.strictObject({
		...
		pilot: z.strictObject({
			...
			billingUnit: z.string().trim().min(1), perAttemptWorstCaseUsd: usdAmountSchema,
			leaseDurationMs: z.number().int().positive(),
			commercialPriceUsd: z.literal("0.00"), commercialReason: z.literal("PILOT_NO_CHARGE"),
		}).optional(),
		...
		keywordSet: z.strictObject({
			...
			keywords: z.array(z.strictObject({
				id: z.string().uuid(), text: z.string().trim().min(1), language: z.string().trim().min(2).max(35),
			})).min(1).optional(),
		}),
	...
	.superRefine((lock, issues) => {
		...
		if (lock.keywordSet.keywords !== undefined) {
			...
			if (snapshots.length !== lock.keywordSet.keywordIds.length || new Set(snapshots.map((item) => item.id)).size !== snapshots.length
				|| snapshots.some((item) => !lock.keywordSet.keywordIds.includes(item.id) || item.language !== lock.request.language))
