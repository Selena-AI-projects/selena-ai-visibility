...
export function materializeLocalMapsProviderRequest(
	...
	if (lock.keywordSet.keywords !== undefined && (!frozen || frozen.text !== keyword.text || frozen.language !== lock.request.language))
...
export const localMapsLiveSubmittedCandidateSchema = z
	...
	.superRefine((input, issues) => {
		...
		if (input.lock.keywordSet.keywords !== undefined && (!frozen || frozen.text !== input.keyword.text || frozen.language !== input.lock.request.language))
		...
		if (!expectedRequest.success || canonicalLocalMapsJson(expectedRequest.data) !== canonicalLocalMapsJson(input.providerRequest))
			issues.addIssue({
				code: "custom",
