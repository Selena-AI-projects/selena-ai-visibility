...
type PublishedReport = {
	...
	documents?: Array<{ id: string; title: string; description: string; href: string }>;
		...
		<main className="min-h-screen bg-[#f7f2ea] px-4 py-8 text-[#181614] sm:px-8">
			<div className="mx-auto max-w-5xl space-y-8">
				<header>
					...
					<a href="/selena/local/checkout" className={`${control} mt-4`}>
						Create a $49 test order
					</a>
				...
				{state === "ready" && report && (
					<>
						...
						{report.documents?.length ? (
							<section aria-labelledby="local-analysis-title" className="space-y-4">
								<h2 id="local-analysis-title" className="text-xl font-semibold">
									Analysis and next steps
								</h2>
								<p>Read the reviewed interpretation and recommendations alongside the measured positions below.</p>
								{report.documents.map((document) => (
									<div key={document.id}>
										<a href={document.href} className={control}>
											{document.title}
										</a>
