...
//
// Who wrote these: this is a fixed dictionary curated with the product owner,
// following how AI crawlers actually read websites (inspired by the
// Cloudflare-style agent-readiness tooling). It is never generated by a
// model at page-render time — the same rule always says the same thing.

type RuleLocale = "en" | "ru";

export type RuleHelpEntry = {
	/** What to do, as a headline. */
	title: string;
	/** Why it matters for AI answers, one plain sentence. */
	why: string;
	/** Concrete steps: what, where on the site, how. */
	steps: string[];
	/** What "done right" looks like, when an example helps. */
	example?: string;
};

export const RULE_HELP: Record<string, { en: RuleHelpEntry; ru: RuleHelpEntry }> = {
	"WEB-001": {
		en: {
			...
			steps: [
				"Open your site editor (Tilda/WordPress/…) → page settings → the “Title” field of the home page.",
				"Write: brand + what you are + where.",
			],
		...
		ru: {
			...
			steps: [
				"Откройте редактор сайта (Tilda/WordPress/…) → настройки главной страницы → поле «Title».",
				"Напишите: бренд + что вы такое + где.",
			],
	...
	"WEB-002": {
		en: {
			...
			steps: [
				"Same page settings → the “Description” field.",
				"1–2 sentences: what the place is, for whom, and the area.",
			],
		...
		ru: {
			...
			steps: [
				"Там же в настройках страницы → поле «Description».",
				"1–2 предложения: что это за место, для кого и в каком районе.",
			],
	...
	"WEB-003": {
		en: {
			...
			steps: [
				"Ask your developer (or use the copy-task button) to add a robots meta tag to the site's pages.",
				"If you want to be found: it should say indexing is allowed.",
			],
		...
		ru: {
			...
			steps: [
				"Попросите разработчика (или используйте кнопку «Скопировать задание») добавить на страницы сайта мета-тег robots.",
				"Если хотите, чтобы вас находили, — в нём должно быть разрешение на индексацию.",
			],
			example: '<meta name="robots" content="index, follow">',
		},
	},
	...
	"WEB-006": {
		en: {
			...
			steps: [
				"In the site editor make the main line of the page a Heading 1 (H1) — the brand and what you are.",
				"Each section (“Menu”, “Location”, “Contacts”) gets a Heading 2 (H2).",
			],
		},
		ru: {
			...
			steps: [
				"В редакторе сайта сделайте главную строку страницы Заголовком 1 (H1) — бренд и что вы такое.",
				"Каждый раздел («Меню», «Как добраться», «Контакты») — Заголовком 2 (H2).",
			],
		},
	},
	"WEB-007": {
		en: {
			...
			steps: [
				"Find what exists only as pictures (menu photo, price list scan).",
				"Duplicate it as plain text on the page: dishes, prices, services.",
			],
		},
		ru: {
			...
			steps: [
				"Найдите, что существует только картинками (фото меню, скан прайса).",
				"Продублируйте это обычным текстом на странице: блюда, цены, услуги.",
			],
		},
	},
	"WEB-008": {
	...
	"WEB-012": {
		en: {
			...
			steps: [
				"Make (or expand) a page that says in plain sentences: what you serve, price range, the area and landmarks.",
				"Use the words customers ask with: the category (“food hall”, “cafe”) and the district — not only the brand name.",
			],
		...
		ru: {
			...
			steps: [
				"Сделайте (или расширьте) страницу, где обычными предложениями написано: что подаёте, диапазон цен, район и ориентиры.",
				"Используйте слова, которыми спрашивают клиенты: категорию («фуд-холл», «кафе») и район — а не только название бренда.",
			],
...
function entry(locale: RuleLocale, ruleId: string): RuleHelpEntry | null {
...
export function ruleSteps(locale: RuleLocale, ruleId: string): string[] {
	return entry(locale, ruleId)?.steps ?? [];
}

export function ruleExample(locale: RuleLocale, ruleId: string): string | null {
	return entry(locale, ruleId)?.example ?? null;
}

/** A ready-to-paste task for a developer or an AI coding agent. */
export function ruleFixTask(locale: RuleLocale, ruleId: string, websiteUrl: string, fallbackTitle: string, fallbackHow: string): string {
