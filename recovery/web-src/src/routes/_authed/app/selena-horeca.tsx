		...
		<div className="selena-app min-h-screen bg-[#ece4d8] pb-16 text-[#181614]">
			...
			<main className="mx-auto grid w-full max-w-7xl gap-8 px-5 py-8 sm:px-8 lg:grid-cols-[17rem_minmax(0,1fr)] lg:py-12">
				...
				<div className="min-w-0">
					{workspace.reportDocuments.length > 0 && (
						<section className="mb-8 border-b border-[#e6ddd1] pb-8" aria-labelledby="report-library-title">
							<h2 id="report-library-title" className="font-serif text-2xl font-semibold">
								{locale === "ru" ? "Готовые отчёты AVLI" : "AVLI reports"}
							</h2>
							<p className="mt-2 text-sm leading-6 text-[#574d45]">
								{locale === "ru"
									? "Сохранённые клиентские документы. Открытие и скачивание не запускают новые измерения."
									: "Saved client documents in Russian. Opening or downloading does not start new measurements."}
							</p>
							<ul className="mt-5 divide-y divide-[#e6ddd1]">
								{workspace.reportDocuments.map((document) => (
									<li key={document.id} className="py-4">
										<h3 className="font-semibold">{document.title}</h3>
										<p className="mt-1 text-sm leading-6 text-[#574d45]">{document.description}</p>
										<div className="mt-2 flex flex-wrap gap-4">
											...
											<a
												...
												className="inline-flex min-h-11 items-center text-[#8f5c34] underline underline-offset-4"
											>
												{locale === "ru" ? "Скачать HTML" : "Download HTML"}
											</a>
										</div>
									</li>
								))}
