import { a as __classPrivateFieldSet, i as __classPrivateFieldGet, n as AnthropicError, o as loggerFor, r as ToolError, s as promiseWithResolvers } from "./sdk-CKsEHCXW.mjs";
...
//#region node_modules/.nitro/vite/services/ssr/assets/node-D73QJoXh.js
(function() {
	try {
		var e = "undefined" != typeof window ? window : "undefined" != typeof global ? global : "undefined" != typeof globalThis ? globalThis : "undefined" != typeof self ? self : {};
		var n = new e.Error().stack;
		n && (e._sentryDebugIds = e._sentryDebugIds || {}, e._sentryDebugIds[n] = "cbf9292f-9176-4abe-9ca5-3b202fa0cfe6", e._sentryDebugIdIdentifier = "sentry-dbid-cbf9292f-9176-4abe-9ca5-3b202fa0cfe6");
	} catch (e) {}
})();
/**
	...
	for (;;) {
		...
		try {
			...
			if (isLink) {
				...
				prefix = path$1.resolve(path$1.dirname(prefix), await fs$1.readlink(prefix));
				continue;
			}
...
async function extractSkillArchive(resp, dest) {
	...
	try {
		...
		for (const entry of await fs$1.readdir(srcRoot)) await fs$1.rename(path$1.join(srcRoot, entry), path$1.join(dest, entry));
	} finally {
...
function betaWriteTool(ctx) {
	return betaTool({
		...
		run: async ({ file_path, content }) => {
			...
			const abs = await resolvePath(ctx, file_path);
			try {
				await fs$1.mkdir(path$1.dirname(abs), {
					recursive: true,
					mode: 493
				});
...
function betaGlobTool(ctx) {
	return betaTool({
		...
		run: async ({ pattern, path: searchPath }) => {
			...
			const realRoot = ctx.unrestrictedPaths ? root : await fs$1.realpath(root).catch(() => root);
			const matches = [];
			try {
...
async function walk(root, rel, fn, signal) {
	...
	async function inner(rel, depth) {
		...
		let entries;
		try {
			entries = await fs$1.readdir(path$1.join(root, rel), { withFileTypes: true });
		} catch {
			return true;
		}
...
//# sourceMappingURL=node-D73QJoXh.mjs.map