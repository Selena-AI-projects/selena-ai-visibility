...
import { a as X_TSS_RAW_RESPONSE, d as getStartOptions, f as isRedirect, g as safeObjectMerge, h as runWithStartContext, i as TSS_SERVER_FUNCTION, l as flattenMiddlewares, m as mergeHeaders, n as TSS_CONTENT_TYPE_FRAMED_VERSIONED, o as X_TSS_SERIALIZED, p as isResolvedRedirect, r as TSS_FORMDATA_CONTEXT, s as createNullProtoObject, t as FrameType, u as getStartContext } from "./createServerFn-CnO8ob2E.mjs";
import { t as getServerFnById } from "../__23tanstack-start-server-fn-resolver-CI5_1tn2.mjs";
import { m as secrets } from "./schema-ejW7s7Gs.mjs";
import { i as reportUnknownSelenaEnv, o as CREDENTIAL_ENV_NAMES } from "./env-D9tfoX6E.mjs";
import { a as requestHandler, i as getResponse } from "./server-44w5PK5b.mjs";
import { B as prepareDecrypt, S as decodeProtectedHeader, t as CompactEncrypt, w as decryptCompact } from "./decode_protected_header-BkI8IgKI.mjs";
import { a as init, i as wrapFetchWithSentry, r as wrapMiddlewaresWithSentry } from "../_libs/sentry__tanstackstart-react.mjs";
import { createHash } from "node:crypto";
//#region node_modules/.nitro/vite/services/ssr/index.js
var ssr_exports = /* @__PURE__ */ __exportAll({
	a: () => decryptSecret,
	c: () => createCsrfMiddleware,
	default: () => server_default,
	i: () => EncryptionKeyError,
	l: () => createMiddleware,
	n: () => refreshCredentialOverlay,
	o: () => encryptSecret,
	r: () => storeCredential,
	s: () => getKeyring,
	t: () => getCredential
...
(function() {
	try {
		...
		n && (e._sentryDebugIds = e._sentryDebugIds || {}, e._sentryDebugIds[n] = "0858b638-9fde-452f-b7d4-4718fcd01f27", e._sentryDebugIdIdentifier = "sentry-dbid-0858b638-9fde-452f-b7d4-4718fcd01f27");
	} catch (e) {}
})();
...
function w(e) {
	let r = createStream(), a = e.getReader(), t = re.bind(null, a);
...
async function getStartManifest(matchedRoutes) {
	const { tsrStartManifest } = await import("../_tanstack-start-manifest_v-DAB1xgQ8.mjs");
...
async function loadEntries() {
	const [routerEntry, startEntry, pluginAdapters] = await Promise.all([
		import("./router-CjEF3k1j.mjs"),
		import("./start-OjzJe8R1.mjs"),
		import("./empty-plugin-adapters-BCQEdHmt.mjs")
	]);
	return {
		routerEntry,
		startEntry,
	...
	const startRequestResolver = async (request, requestOpts) => {
		...
		try {
			...
			const requestStartOptions = {
				...startOptions,
				requestMiddleware: hasStartInstance ? startOptions.requestMiddleware : wrapMiddlewaresWithSentry({ defaultCsrfMiddleware }),
...
var KEY_BYTES = 32;
var ENCRYPTION_KEY_ENV = "ELMO_ENCRYPTION_KEY";
var RETIRED_KEYS_ENV = "ELMO_ENCRYPTION_KEY_OLD";
...
*  with no current one). */
var EncryptionKeyError = class extends Error {
	constructor(message) {
		super(message);
...
async function encryptSecret(plaintext, opts) {
	return new CompactEncrypt(new TextEncoder().encode(plaintext)).setProtectedHeader({
		alg: "dir",
		enc: "A256GCM",
		kid: keyId(opts.key),
		ctx: opts.aad
...
async function decryptSecret(payload, opts) {
	const kid = readKeyId(payload);
	const key = opts.keyring.byId.get(kid);
	if (!key) throw new UnknownKeyError(kid);
	try {
		const { plaintext, protectedHeader } = await compactDecrypt(payload, key, {
...
function decodeKey(raw, source) {
	...
	if (key.length !== KEY_BYTES) throw new EncryptionKeyError(`${source} must be base64 for exactly ${KEY_BYTES} bytes (decoded to ${key.length})`);
	return key;
}
...
function getKeyring(env = process.env) {
	const retired = (env["ELMO_ENCRYPTION_KEY_OLD"] ?? "").split(",").map((part) => part.trim()).filter((part) => part.length > 0);
	const raw = env[ENCRYPTION_KEY_ENV];
	if (typeof raw !== "string" || raw.trim().length === 0) {
		if (retired.length > 0) throw new EncryptionKeyError(`${RETIRED_KEYS_ENV} is set without ${ENCRYPTION_KEY_ENV}`);
		return null;
	}
	const primary = decodeKey(raw.trim(), ENCRYPTION_KEY_ENV);
	const byId = /* @__PURE__ */ new Map([[keyId(primary), primary]]);
...
async function refreshCredentialOverlay() {
	let keyring = null;
	try {
		keyring = getKeyring();
	} catch (e) {
	...
	const { db } = await import("./db-DcHqq7B9.mjs").then((n) => n.n).then((n) => n.n);
	...
	for (const row of rows) {
		...
		try {
			next.set(row.name, await decryptSecret(row.encryptedValue, {
				keyring,
				aad: aadForName(row.name)
			}));
		} catch (e) {
			console.error(e instanceof UnknownKeyError ? `[secrets] stored secret "${row.name}" is ${e.message} — restore that key there to finish the rotation` : `[secrets] stored secret "${row.name}" will not decrypt under the key it names — it may be corrupt or tampered with`);
		}
	}
	overlay.clear();
...
async function encryptCredential(name, value) {
	if (!CREDENTIAL_ENV_NAMES.has(name)) throw new Error(`"${name}" is not an overridable credential`);
	const keyring = getKeyring();
	if (!keyring) throw new EncryptionKeyError(`${ENCRYPTION_KEY_ENV} is not set — cannot store encrypted credentials`);
...
async function storeCredential(name, value) {
...
var CREDENTIAL_REFRESH_INTERVAL_MS = 6e4;
...
async function startCredentialRefresh() {
...
function createRateLimiter() {
	...
	return {
		...
		check(key, policy, now = Date.now()) {
			...
			if (window.count >= policy.max) return {
				allowed: false,
...
*/
function policyForRequest(request) {
	...
	if (pathname.startsWith("/api/auth/")) return [
		...
		"change-password",
		"change-email"
	].some((segment) => pathname.includes(segment)) ? AUTH_POLICY : null;
...
function callerKey(request) {
	const credential = (request.headers.get("authorization") ?? request.headers.get("x-api-key") ?? "").replace(/^Bearer\s+/i, "").trim();
...
var strictTransportSecurity = process.env.DEPLOYMENT_MODE === "whitelabel" ? "max-age=63072000" : "max-age=63072000; includeSubDomains";
...
var server_default = createServerEntry(wrapFetchWithSentry({ async fetch(request) {
	...
	return addSecurityHeaders(await server_default$1.fetch(request));
} }));
//#endregion
export { encryptSecret as a, refreshCredentialOverlay as c, decryptSecret as i, ssr_exports as l, createCsrfMiddleware as n, getCredential as o, createMiddleware as r, getKeyring as s, EncryptionKeyError as t, storeCredential as u };
