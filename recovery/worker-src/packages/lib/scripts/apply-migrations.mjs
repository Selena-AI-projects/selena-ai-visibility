...
const APPLIED_MIGRATION_HASH_ALIASES = new Map([
	...
	// The applied 0060 widens sv_run_permits_status_check to admit 'cancelled',
	// with the guard function's matching arm. No release code writes or reads a
	// fourth permit status, so that database is permissive where the release is
	// strict rather than behaving differently.
	[
...
export function assertMigrationApproval({ actualRows, expectedRows, env, log = console.log }) {
	const applied = actualRows ? assertJournalPrefix(actualRows, expectedRows) : 0;
	const pending = expectedRows.length - applied;
	if (pending <= 0) return { pending: 0, gated: false };
	if (!actualRows?.length) return { pending, gated: false };
...
export function assertJournalPrefix(actualRows, expectedRows) {
	let expectedIndex = 0;
	const shippedTimestamps = new Set(expectedRows.map((row) => row.createdAt));
	...
	for (const actual of actualRows) {
		...
		if (
			!shippedTimestamps.has(actual.createdAt) &&
			!seenHistorical.has(actual.createdAt) &&
			STAGING_HISTORICAL_JOURNAL_ROWS.some((row) => row.createdAt === actual.createdAt && row.hash === actual.hash)
		) {
			seenHistorical.add(actual.createdAt);
			continue;
		}
...
export function assertJournalPostcondition(actualRows, expectedRows) {
	...
	const expectedCount = assertJournalPrefix(actualRows, expectedRows);
