...
describe("bounded migration journal acceptance", () => {
	it("does not let recovered history hide pending DDL from owner approval", () => {
		const historical = {
			createdAt: "1787940028001",
			hash: "21ebc5b9378ef1af64c95cac1870882adf52a4ad3c5d3c9c108d208a0634ee45",
		};
		const env = { RAILWAY_ENVIRONMENT_NAME: "staging", RAILWAY_GIT_COMMIT_SHA: "a".repeat(40) };
		const actualRows = [expected[0], historical];
		expect(() => assertMigrationApproval({ actualRows, expectedRows: expected, env, log: () => {} })).toThrow(
			"SELENA_MIGRATION_OWNER_APPROVAL_REQUIRED",
		);
		expect(
			assertMigrationApproval({
				actualRows,
				expectedRows: expected,
				env: { ...env, [MIGRATION_APPROVAL_ENV]: "a".repeat(40) },
				log: () => {},
			}),
		).toEqual({ pending: 2, gated: true });
		expect(() =>
			assertMigrationApproval({ actualRows: [historical], expectedRows: expected, env, log: () => {} }),
		).toThrow("SELENA_MIGRATION_OWNER_APPROVAL_REQUIRED");
	});

	it("counts recovered Local migrations when they are present in the unified source journal", () => {
		const local = {
			createdAt: "1787940028001",
			hash: "21ebc5b9378ef1af64c95cac1870882adf52a4ad3c5d3c9c108d208a0634ee45",
		};
		const unified = [...expected, local];
		expect(() => assertJournalPostcondition(unified, unified)).not.toThrow();
		expect(() => assertJournalPostcondition([...unified, local], unified)).toThrow(
			"SELENA_MIGRATION_CEILING_ALREADY_EXCEEDED",
		);
		expect(() => assertJournalPostcondition([local, ...expected], unified)).toThrow(
			"SELENA_MIGRATION_JOURNAL_MISMATCH",
		);
		expect(() => assertJournalPostcondition([...expected, local, local], expected)).toThrow(
			"SELENA_MIGRATION_CEILING_ALREADY_EXCEEDED",
		);
		expect(() => assertJournalPostcondition(expected, unified)).toThrow("SELENA_MIGRATION_POSTCONDITION_FAILED");
	});
