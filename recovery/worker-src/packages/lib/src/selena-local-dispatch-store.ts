...
export function createLocalDispatchStore(db: OrganizationDatabase, organizationId: string, leaseMs: number) {
	if (!organizationId.trim() || !Number.isSafeInteger(leaseMs) || leaseMs <= 0) throw new Error("LOCAL_OUTBOX_SCOPE_REQUIRED");
	return {
		async claim(input: { token: string; limit: number }): Promise<Claim[]> {
			if (!Number.isInteger(input.limit) || input.limit < 1 || input.limit > 9) throw new Error("LOCAL_OUTBOX_LIMIT_INVALID");
			return withOrganizationTransaction(db, organizationId, async (tx) => {
				...
				return (result.rows as Array<Record<string, string>>).map((row) => ({ id: row.id, claimToken: input.token,
					data: { organizationId: row.organization_id, measurementCycleId: row.measurement_cycle_id, localCycleId: row.local_cycle_id, observationId: row.observation_id, attemptId: row.attempt_id } }));
			});
		},
		...
		async markEnqueued(claim: Claim): Promise<void> {
			...
			await withOrganizationTransaction(db, organizationId, async (tx) => {
				const result = await tx.execute(sql`update sv_local_dispatch_outbox set status='ENQUEUED',claim_token=null,lease_expires_at=null,enqueued_at=now()
