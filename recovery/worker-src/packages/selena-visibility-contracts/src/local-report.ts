...
export const localPilotCycleStatuses = [
	"CREATED", "PREFLIGHT_BLOCKED", "BUDGET_BLOCKED", "APPROVED", "CANARY_RUNNING", "CANARY_REVIEW",
	"QUEUED", "RUNNING", "PARTIAL_FAILURE", "PROVIDER_BLOCKED", "UNKNOWN_RECONCILIATION", "STOPPED",
	"CARDINALITY_INCIDENT", "QC_REQUIRED", "READY", "COMPLETED", "FAILED",
] as const;
export const localPilotExecutionModes = ["LEGACY_SOURCE_ONLY", "CANARY", "PILOT"] as const;
export const localPilotObservationOutcomes = [
	"PENDING", "FOUND", "ABSENT_WITHIN_DEPTH", "INVALID", "UNKNOWN", "BLOCKED", "CANCELLED",
] as const;
...
export const localProviderContractSchema = z.strictObject({
	...
	price: z.strictObject({
		billingUnit: z.string().trim().min(1), currency: z.literal("USD"),
		perAttemptWorstCaseUsd: usd, priceSnapshotVersion: z.string().trim().min(1),
	}),
});
...
export const localPilotObservationSchema = z.strictObject({
	id: z.string().uuid(),
	...
	switch (row.outcome) {
		...
		case "ABSENT_WITHIN_DEPTH":
			valid = row.validity === "VALID" && row.capturedAt !== null && row.reason === null && row.evidenceId !== null
		...
		case "BLOCKED":
		case "CANCELLED":
			valid = row.validity === "UNMEASURED" && row.targetRank === null && row.capturedAt === null && row.evidenceId === null
...
export const localPilotEvidenceEnvelopeSchema = z.strictObject({
	schemaVersion: z.literal(1),
	canonicalizationVersion: z.literal("canonical-json-code-unit-v1"),
	storageClass: z.enum(["CANARY_ONLY", "PILOT"]),
	organizationId: z.string().trim().min(1),
	measurementCycleId: z.string().uuid(), localCycleId: z.string().uuid(), observationId: z.string().uuid(),
	attemptId: z.string().uuid(), configurationLockId: z.string().uuid(),
	providerContractDigest: sha256,
	providerTaskId: z.string().trim().min(1),
	keyword: z.strictObject({ id: z.string().uuid(), text: z.string().trim().min(1), language: z.string().trim().min(2) }),
	point: z.strictObject({ id: z.string().uuid(), latitude: z.number().min(-90).max(90), longitude: z.number().min(-180).max(180) }),
	depth: z.literal(20), outcome: z.enum(["FOUND", "ABSENT_WITHIN_DEPTH"]),
	targetRank: z.number().int().min(1).max(20).nullable(),
	capturedAt: z.iso.datetime(), rawReference: z.string().trim().min(1), rawSha256: sha256,
	actualCostUsd: usd,
